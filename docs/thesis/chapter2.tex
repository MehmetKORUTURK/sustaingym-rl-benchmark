...
\paragraph{Empirical studies.} Several studies have empirically investigated the effect of observation noise on RL performance.~\citet{dulacarnold2021challenges} identified observation noise as one of the key challenges for real-world RL and proposed benchmarking protocols for evaluating noise robustness. Their work highlighted that even moderate levels of observation noise can cause significant performance degradation in standard RL algorithms. \citet{fox2018dithering} studied the interaction between observation noise and function approximation errors in deep RL, showing that these two sources of error can compound multiplicatively.
...
\paragraph{Our approach to distribution shift.} In this thesis, we study distribution shift by training policies under varying noise levels and evaluating their robustness to deployment-time perturbations. We characterize noise sensitivity through systematic degradation curves across observation, action, and environment noise channels, and investigate whether noise during training provides implicit regularization benefits.
