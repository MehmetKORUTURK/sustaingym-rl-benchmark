...
\section{Perturbation Models and Robustness Theory}
\label{sec:noise_models}

A central contribution of this thesis is the systematic characterization of RL algorithm robustness to perturbation. This section formalizes the three perturbation channels used in our framework.

\subsection{State Perturbation Model}

State perturbation models sensor uncertainty and measurement errors. Given the true state $s_t$, the agent observes a perturbed version $\tilde{s}_t$:
\begin{equation}
    \tilde{s}_t = h_{\text{obs}}(s_t, \xi_t^{\text{obs}}),
    \label{eq:obs_noise}
\end{equation}
where $\xi_t^{\text{obs}}$ is the perturbation random variable and $h_{\text{obs}}$ is the perturbation injection function. We consider two common models:

\begin{definition}[Additive Gaussian Noise]
\begin{equation}
    \tilde{s}_t = s_t + \epsilon_t, \quad \epsilon_t \sim \mathcal{N}(0, \sigma_s^2 I),
\end{equation}
where $\sigma_s$ is the state perturbation standard deviation. This model is used for MOER values and departure estimates in the EV charging environment.
\end{definition}

\begin{definition}[Proportional (Multiplicative) Gaussian Noise]
\begin{equation}
    \tilde{s}_t = s_t \odot (1 + \epsilon_t), \quad \epsilon_t \sim \mathcal{N}(0, \sigma_s^2 I),
\end{equation}
where $\odot$ denotes element-wise multiplication. This model scales the perturbation magnitude with the signal magnitude, which is physically appropriate for percentage-based sensor errors. This model is used for the building environment observations and cogeneration environment sensor readings.
\end{definition}

\paragraph{Mixed perturbation.} In the EV charging environment, different observation components use different perturbation models:
\begin{align}
...
\end{align}
Note the factor of 50 applied to departure time perturbation, reflecting the larger scale and higher uncertainty of departure estimates compared to MOER values.

\subsection{Action Perturbation Model}

Action perturbation models actuator imprecision and command execution errors. Given the policy output $a_t = \pi(s_t)$, the executed action is:
\begin{equation}
...
\begin{equation}
    \tilde{a}_t = \clip(a_t + \epsilon_t, a_{\min}, a_{\max}), \quad \epsilon_t \sim \mathcal{N}(0, \sigma_a^2 I).
\end{equation}

\paragraph{Multiplicative Gaussian noise (continuous actions):}
\begin{equation}
    \tilde{a}_t = \clip(a_t + a_t \odot \epsilon_t, a_{\min}, a_{\max}), \quad \epsilon_t \sim \mathcal{N}(0, \sigma_a^2 I).
\end{equation}
...
\subsection{Dynamics Perturbation Model}

Dynamics perturbation models stochastic perturbations in the system dynamics that are not directly observable to the agent. Given the true environment parameters $\theta_{\text{env}}$, the perturbed parameters are:
\begin{equation}
    \tilde{\theta}_{\text{env}} = \theta_{\text{env}} + \epsilon_{\text{env}}, \quad \epsilon_{\text{env}} \sim \mathcal{N}(0, \Sigma_d).
    \label{eq:env_noise}
\end{equation}

Dynamics perturbation affects the dynamics and/or reward computation but is not reflected in the agent's observations. Specific implementations:
\begin{itemize}
...
\subsection{Perturbation Channel Independence}

A key design principle of our perturbation framework is \textbf{channel independence}: state perturbation, action perturbation, and dynamics perturbation are sampled independently at each timestep.

\begin{assumption}[Perturbation Channel Independence]
...
This independence enables controlled experiments where the contribution of each perturbation channel can be isolated through ablation studies.
...
Each environment is described in full technical detail in \Cref{ch:methodology}, including the physical models, observation and action space definitions, reward formulations, and perturbation injection mechanisms.
