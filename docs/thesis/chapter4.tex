...
\node[layer, fill=blue!8] (env) at (0, 0) {
    ...
    {\scriptsize Gymnasium API \,$\cdot$\, configurable noise params \,$\cdot$\, MARL via PettingZoo}
};
\node[layerlabel] at (-7.5, 0) {Environment\\Layer};

% Layer 2: Noise Injection
\node[layer, fill=orange!10] (noise) at (0, 2.4) {
    \textbf{Observation Noise} \quad\quad \textbf{Action Noise} \quad\quad \textbf{Environment Noise}\\[2pt]
    {\scriptsize sensor uncertainty \hspace{1.5cm} actuator imprecision \hspace{1.2cm} stochastic dynamics}
};
\node[layerlabel] at (-7.5, 2.4) {Noise Injection\\Layer};
...
\node[layer, fill=purple!8] (eval) at (0, 7.2) {
    ...
    {\scriptsize 1000 episodes \hspace{1.5cm} distributional analysis \hspace{1cm} convergence \hspace{1.5cm} noise sensitivity}
};
...
\node[block, fill=blue!10] (config) at (0, 0) {Config\\{\scriptsize algo, env, noise}};
...
\draw[darrow] (config.south) -- ++(0,-0.6) node[below, font=\scriptsize, text=gray!60!black] {noise params: $\sigma_{\text{obs}}, \sigma_{\text{act}}, \sigma_{\text{env}}$};

\end{tikzpicture}
\caption{Experimental workflow. Each configuration specifies an algorithm, environment, and noise level. SLURM dispatches training jobs to ARC HPC, producing checkpoints that are evaluated over 1000 episodes. Outputs include summary statistics, distributional violin plots, and training convergence curves.}
\label{fig:workflow}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Complete experiment matrix. Standard RL trains each algorithm under three independent noise channels (observation, action, environment) at varying intensities.}
