...
We designed and implemented multi-agent versions of all three environments using the PettingZoo parallel interface with physically motivated agent decompositions: per-station agents in EV Charging (54 agents), per-zone agents in Building HVAC (5 agents), and per-turbine agents in Cogeneration (4 agents). Training with Ray RLlib using PPO, SAC, APPO, and IMPALA under both independent and shared policy modes yielded three key findings:

\begin{enumerate}
    \item \textbf{Off-policy methods dominate multi-agent coordination.} SAC is the only algorithm that learns in Building MARL (converging to $-45$ vs the DoNothing baseline of $-288$ for all on-policy methods), and IMPALA's V-trace correction achieves 45\% lower cost than PPO/APPO in Cogeneration. Experience replay and importance-weighted corrections are critical for multi-agent credit assignment.

    \item \textbf{MARL viability is environment-dependent.} EV Charging's cleanly decomposable reward structure allows all algorithms to achieve within 8\% of single-agent performance, while Building's thermally coupled RC dynamics cause catastrophic failure for on-policy methods. Environment coupling structure---not agent count---determines MARL feasibility.

    \item \textbf{Parameter sharing has negligible impact.} Shared vs independent policy modes produce statistically indistinguishable results across all three environments and all tested algorithms, suggesting that algorithm choice (on-policy vs off-policy) dominates over policy architecture in these energy system environments.
\end{enumerate}
