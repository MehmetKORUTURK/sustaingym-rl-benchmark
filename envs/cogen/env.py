...
class CogenEnv(gym.Env):
    ...
    def __init__(self,
                 ...
                 noise_env: dict | float | None = None,
                 ):
        ...
        # Environmental noise: affects ambient conditions fed to the ONNX
        # model AND reward computation (not just observations).
        # "model" key: multiplicative noise on ONNX outputs (sim-to-real gap)
        _default = {"temperature": 0.0, "pressure": 0.0, "humidity": 0.0, "model": 0.0}
        if noise_env is None:
            self.noise_env = _default
        elif isinstance(noise_env, dict):
            self.noise_env = {**_default, **noise_env}
        else:
            self.noise_env = {
                "temperature": float(noise_env),
                "pressure": float(noise_env),
                "humidity": float(noise_env),
            }
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None
        ...
        return_obs = self._apply_env_noise(self.obs)
        info = {
    ...
    def _apply_env_noise(self, obs: dict[str, Any]) -> dict[str, Any]:
        """Apply environmental noise to ambient conditions.

        Returns a deep copy with noisy TAMB[0], PAMB[0], RHAMB[0].
        This affects the ONNX model inputs and therefore the reward.
        The internal ``self.obs`` is never mutated.
        """
        if all(v == 0.0 for v in self.noise_env.values()):
            return obs
        ...
        if self.noise_env["temperature"] > 0.0 and "TAMB" in obs:
            ...
            obs["TAMB"][0] += self.np_random.normal(
                0, self.noise_env["temperature"])

        if self.noise_env["pressure"] > 0.0 and "PAMB" in obs:
            obs["PAMB"] = obs["PAMB"].copy()
            obs["PAMB"][0] += self.np_random.normal(
                0, self.noise_env["pressure"])

        if self.noise_env["humidity"] > 0.0 and "RHAMB" in obs:
            obs["RHAMB"] = obs["RHAMB"].copy()
            obs["RHAMB"][0] += self.np_random.normal(
                0, self.noise_env["humidity"])
    ...
    def _compute_reward(self, obs: dict[str, Any], action: dict[str, Any]
        ...
        # Model uncertainty noise: multiplicative perturbation on ONNX outputs
        # Simulates sim-to-real gap (surrogate model vs actual plant dynamics)
        model_noise_std = self.noise_env.get("model", 0.0)
        if model_noise_std > 0.0:
            noise = self.np_random.normal(0, model_noise_std, size=model_output.shape)
            model_output = model_output * (1.0 + noise)
    ...
    def step(self, action: dict[str, Any]
        ...
        Returns:
            ...
            info: info dict
        """
        # Apply environmental noise *before* reward so the ONNX model
        # and penalty terms see perturbed ambient conditions.
        obs_for_reward = self._apply_env_noise(self.obs)
        self.current_reward, self.current_info = self._compute_reward(
            obs_for_reward, action)
        ...
        # update the current observation (true internal state stays clean)
        self.obs = self._get_obs()

        # Return a noisy copy so the agent also sees env variations
        return_obs = self._apply_env_noise(self.obs)
        ...
        return return_obs, self.current_reward, terminated, truncated, self.current_info
