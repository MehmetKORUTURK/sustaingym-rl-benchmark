...
class MultiAgentCogenEnv(ParallelEnv):
    ...
    def __init__(self,
        ...
        # Use -inf/+inf bounds because ONNX model outputs can exceed
        # the nominal observation_space bounds from CogenEnv.
        ...
        wide_observation_space = spaces.Box(
            low=-np.inf, high=np.inf,
            shape=flat_observation_space.shape,
            dtype=flat_observation_space.dtype,
        )
        self.observation_spaces = {
            agent: wide_observation_space for agent in self.agents
        }
    ...
    def step(self, actions: dict[str, dict[str, np.ndarray]]
        ...
        for agent in self.agents:
            ...
            # Scale reward by 1e-7 to match MyCogenEnv (single-agent)
            # Raw costs are in dollars ($100-10000+/step), unscaled → gradient explosion
            rewards[agent] = -(
                info['fuel_costs'][agent]
                + info['ramp_costs'][agent]
                + info['dyn_cv_costs'][agent]
                + info['non_delivery_cost'] / self.num_agents) / 1e7
