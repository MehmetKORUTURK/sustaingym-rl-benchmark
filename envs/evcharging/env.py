...
class EVChargingEnv(Env):
    ...
    def __init__(self, data_generator: AbstractTraceGenerator,
        ...
        if self.safe_rl:
            self.observation_space = spaces.Box(
                low=np.array([low for v in self.observation_space.values() for low in v.low]),
                high=np.array([high for v in self.observation_space.values() for high in v.high]),
                dtype=np.float32
            )
    ...
    def _init_action_projection(self) -> None:
        """Initializes optimization problem, parameters, and variables with safety constraints."""
        ...
        # Parameters to be set when stepping through the environment
    ...
    def step(self, action: np.ndarray
        ...
        # Apply observation noise using seeded RNG for reproducibility
        # IMPORTANT: Create deep copy to avoid mutating internal state arrays
        if self.noise is not None:
            ...
            # With noise=0.1, this gives ~5 timesteps (~25 minutes) of departure time uncertainty
            eps_dep = self.np_random.normal(0, self.noise * 50, size=observation['est_departures'].shape)
            observation['est_departures'][active] = np.clip(
                observation['est_departures'][active] + eps_dep[active], -288, 288
            )

            # Demands (relative noise with bias prevention)
            # Clip multiplier to prevent upward bias from negative outliers being clamped to 0
            # ±50% bounds are physically reasonable for battery SOC estimation uncertainty
            eps_dem = self.np_random.normal(0, self.noise, size=observation['demands'].shape)
            multiplier = np.clip(1 + eps_dem, 0.5, 1.5)
            observation['demands'][active] = np.clip(
                observation['demands'][active] * multiplier[active],
                0,
        ...
        info = self._get_info()
        # print(f"Action at step {self.t}: {np.sum(action*self.ACTION_SCALE_FACTOR)}")
    ...
    def _to_schedule(self, action: np.ndarray) -> dict[str, list[float]]:
        ...
        # Convert normalized action [0,1] to pilot signals in Amps [0,32]
        action *= self.ACTION_SCALE_FACTOR
        pilot_signals = {}
    ...
    def _get_reward(self, schedule: Mapping[str, Sequence[float]]) -> float:
        ...
        # Carbon cost with optional environmental disturbance (Amp * period * kg CO2 / kWh) -> ($)
        actual_moer = self.moer[self.t, 0]
        if self.noise_env is not None:
            # Add real-time grid volatility (renewable fluctuations, demand spikes)
            moer_disturbance = self.np_random.normal(0, self.noise_env)
            actual_moer = np.clip(actual_moer + moer_disturbance, 0, 1)
        carbon_cost = self.CARBON_COST_FACTOR * total_charging_rate * actual_moer
